...
<?php return; endif;
$summary = $pdo->query("SELECT COUNT(DISTINCT p.user_id) active_users, COUNT(p.id) total_pickups, ROUND(COALESCE(SUM(p.estimated_weight*COALESCE(ef.co2_sa_adjusted,ca.avg_co2,0)),0),2) co2, ROUND(COALESCE(SUM(p.estimated_weight*COALESCE(ef.water_liters_per_kg,ca.avg_water_liters_per_kg,0)),0),2) water, ROUND(COALESCE(SUM(p.estimated_weight*COALESCE(ef.energy_kwh_per_kg,ca.avg_energy_kwh_per_kg,0)),0),2) energy FROM pickups p LEFT JOIN emission_factors ef ON ef.category=p.category AND p.subcategory IS NOT NULL AND p.subcategory<>'' AND ef.subcategory=p.subcategory LEFT JOIN category_averages ca ON ca.category=p.category WHERE p.status='completed'")->fetch();
$categories = $pdo->query("SELECT p.category, ROUND(SUM(p.estimated_weight),2) kg, ROUND(SUM(p.estimated_weight*COALESCE(ef.co2_sa_adjusted,ca.avg_co2,0)),2) co2 FROM pickups p LEFT JOIN emission_factors ef ON ef.category=p.category AND p.subcategory IS NOT NULL AND p.subcategory<>'' AND ef.subcategory=p.subcategory LEFT JOIN category_averages ca ON ca.category=p.category WHERE p.status='completed' GROUP BY p.category ORDER BY co2 DESC")->fetchAll();
?>
